...
- Критерий χ² показывает согласие распределения выборки с теоретическим. Для больших N p-значение > 0.05. При этом можно заметить(как и в случае дискретной случайной величины), что иногда при N=10000 p-value может опуститься ниже 0.05, тогда как при N=100,1000 такого не происходит. Это показывает, что даже малейшие отклонения от теоретического распределения при большом объеме выборки могут приводить к росту хи квадрат.  
